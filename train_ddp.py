...
def train_loop(
        rank, epoch, check_unit,
        model: DDP, optimizer: torch.optim.Optimizer,
...
def _ddp_setup(rank, world_size, config: DictConfig, train_set, val_set):
    distributed.init_process_group(
        backend="nccl", init_method="tcp://localhost:12355",
        rank=rank, world_size=world_size,
    )

    train_sampler = DistributedSampler(train_set, world_size, rank)
    train_loader = DataLoader(
        train_set, pin_memory=True, sampler=train_sampler,
    ...
    return (train_loader, val_loader, early_stopping, early_stop, check_unit, device, run)
...
def _ddp_train_worker(
    ...
    for epoch in range(config.epochs):
        ...
        if rank == 0:
            ...
            if improved is True:
                torch.save(model.state_dict(), f"{config.model_save_path}/{config.model_name}_epoch{epoch:2d}.pth")
